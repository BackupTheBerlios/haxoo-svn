...
class SmartpartnerPartnerHandler extends XoopsObjectHandler
{
	...
	*/
	function insert(&$partner, $force = false)
	{
		...
		if ($partner->isNew()) {
			$sql = sprintf("INSERT INTO %s (id, weight, hits, hits_page, url, image, image_url, title, summary, description, contact_name, contact_email, contact_phone, adress, `status`, domains, tags) VALUES ('', %u, %u, %u, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %u, %s, %s)", $this->_db->prefix($this->_dbtable), $weight, $hits, $hits_page, $this->_db->quoteString($url), $this->_db->quoteString($image), $this->_db->quoteString($image_url), $this->_db->quoteString($title), $this->_db->quoteString($summary), $this->_db->quoteString($description), $this->_db->quoteString($contact_name), $this->_db->quoteString($contact_email), $this->_db->quoteString($contact_phone), $this->_db->quoteString($adress), $status, $this->_db->quoteString($domains), $this->_db->quoteString($tags));
		} else {
			$sql = sprintf("UPDATE %s SET weight = %u, hits = %u, hits_page = %u, url = %s, image = %s, image_url = %s, title = %s, summary = %s, description = %s, contact_name = %s, contact_email = %s, contact_phone = %s, adress = %s, `status` = %u, domains = %s, tags = %s WHERE id = %u", $this->_db->prefix($this->_dbtable), $weight, $hits, $hits_page, $this->_db->quoteString($url), $this->_db->quoteString($image), $this->_db->quoteString($image_url), $this->_db->quoteString($title), $this->_db->quoteString($summary), $this->_db->quoteString($description), $this->_db->quoteString($contact_name), $this->_db->quoteString($contact_email), $this->_db->quoteString($contact_phone), $this->_db->quoteString($adress), $this->_db->quoteString($domains), $this->_db->quoteString($tags), $status, $id);
		}
	...
	*/
	...
	function getPartners($limit=0, $start=0, $status=_SPARTNER_STATUS_ACTIVE, $sort='title', $order='ASC', $asobject=true, $admin=false)
	{
		global $xoopsUser;
		If ($status != _SPARTNER_STATUS_ALL) {
			$criteriaStatus = new CriteriaCompo();
