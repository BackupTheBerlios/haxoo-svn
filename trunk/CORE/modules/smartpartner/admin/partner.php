...
function editpartner($showmenu = false, $id = 0)
{
	...
	if (file_exists($GLOBALS['xoops']->path('/modules/tag/include/formtag.php'))) {
		...
		$tags_text = new XoopsFormTag("tags", 60, 255, $partnerObj->id(), $catid = 0);
		$sform->addElement($tags_text, false);	
	}
	...
	if (file_exists($GLOBALS['xoops']->path('/modules/multisite/class/formselectdomains.php'))) {
		...
		$domains_text = new XoopsFormSelectDomains(_CO_SPARTNER_DOMAINS, "domains", $partnerObj->getVar('domains'), 10, true);
		$domains_text->setDescription(_CO_SPARTNER_DOMAINS_DSC);
		$sform->addElement($domains_text, false);	
	}
