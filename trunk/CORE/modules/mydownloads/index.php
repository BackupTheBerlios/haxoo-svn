<?php
// $Id: index.php,v 1.14 2008/01/22 17:01:03 yoshis Exp $
//  ------------------------------------------------------------------------ //
//                             mydownloads_fileup                            //
...
//
// redirect by bluemoon inc.
//
$title = "";
foreach($_GET as $k=>$v){
	$title = urlencode(strip_tags($k));
}
if (strlen($title)>0) {
	$sql = "SELECT cid,lid FROM ".$xoopsDB->prefix("mydownloads_downloads")
		." WHERE title LIKE '" . $title . "' AND status>0;";
	if ( $result = $xoopsDB->query( $sql )){
		list( $cid, $lid ) = $xoopsDB->fetchRow( $result );
		if ( $cid && $lid ) redirect_header("singlefile.php?cid=$cid&lid=$lid", 0, "Jump to download");
	}
}
// redirect end
...
//$xoopsTpl->assign('show_logo' , $xoopsModuleConfig['show_logo']);
$xoopsTpl->assign('show_option' , $xoopsModuleConfig['show_option']);
$dlfile = array();
$sql = "SELECT d.lid, d.cid, d.title, d.url, d.homepage, d.version, d.size, d.platform, d.logourl, d.status, d.date, d.hits, d.rating, d.votes, d.comments, t.description FROM "
	.$xoopsDB->prefix("mydownloads_downloads")." d LEFT JOIN ".$xoopsDB->prefix("mydownloads_text")." t ON t.lid=d.lid WHERE d.status>0 ORDER BY date DESC";
$result = $xoopsDB->query($sql, $xoopsModuleConfig['newdownloads'], 0);
while(list($lid, $cid, $dtitle, $url, $homepage, $version, $size, $platform, $logourl, $status, $time, $hits, $rating, $votes, $comments, $description)=$xoopsDB->fetchRow($result)) {
	...
	$dlfile[] = array(
		...
		'version'  => $version,
		'category'  => $path,
		'lang_dltimes' => sprintf(_MD_DLTIMES,$hits),
		'mail_subject' => rawurlencode(sprintf(_MD_INTFILEFOUND,$xoopsConfig['sitename'])),
		'mail_body' => rawurlencode(sprintf(_MD_INTFILEFOUND,$xoopsConfig['sitename']).':  '.XOOPS_URL.'/modules/mydownloads/singlefile.php?cid='.$cid.'&amp;lid='.$lid)
	);
}
$xoopsTpl->assign('file',$dlfile);
