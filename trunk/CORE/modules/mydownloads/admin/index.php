...
function mydownloads()
{
	...
	/*
	if (intval($xoopsModule->getVar('version')) < 120) {
	echo "<li id='" . $tblColors[7] . "'><a href=\"upgrade.php\"><span>" . _AM_MYDL_PLEASE_UPGRADE . "</span></a></li>\n";
	}*/
	...
	if( defined( 'XOOPS_CUBE_LEGACY' ) ) {
		// Cube Legacy
		$prefURL = '/modules/legacy/admin/index.php?action=PreferenceEdit&confmod_id=';
	} else {
		// conventinal X2
		$prefURL = '/modules/system/admin.php?fct=preferences&op=showmod&mod=';
	}

	echo " - <a href='".XOOPS_URL.$prefURL.$xoopsModule->getVar('mid')."'>"._MD_GENERALSET."</a>";
	echo "<br /><br />";
    ...
    if($totalmodrequests>0){
        ...
        while(list($requestid, $lid, $cid, $title, $url, $homepage, $version, $size, $platform, $logourl, $description, $modifysubmitter)=$xoopsDB->fetchRow($result)) {
            ...
            if ( $xoopsModuleConfig['useshots'] && !empty($origlogourl) ){
                echo "<img src=\"".XOOPS_URL."/modules/mydownloads/images/shots/".$origlogourl."\" width=\"".intval($xoopsModuleConfig['shotwidth'])."\" />";
            }else{
                echo "&nbsp;";
            }
            echo "</td></tr>
            ...
            if ( $xoopsModuleConfig['useshots'] && !empty($logourl) ){
                echo "<img src=\"".XOOPS_URL."/modules/mydownloads/images/shots/".$logourl."\" width=\"".intval($xoopsModuleConfig['shotwidth'])."\" />";
            } else {
                echo "&nbsp;";
            }
            echo "</td></tr>
