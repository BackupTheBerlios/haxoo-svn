...
if (empty($_POST["pdf_data"])) 
{
    ...
    $pdf_data['content'] = utf8_decode($post_data['text']);
    $pdf_data['author'] = $post_data['author'];
    $pdf_data['url'] = XOOPS_URL."modules/newbb/viewtopic.php?post_id=".$post_id;;
...
$pdf_data['title'] = NEWBB_PDF_SUBJECT.': '.utf8_decode($pdf_data["title"]);
if (!empty($pdf_data['subtitle']))
{
    $pdf_data['subtitle'] = NEWBB_PDF_TOPIC.': '.utf8_decode($pdf_data['subtitle']);
}
$pdf_data['author'] = NEWBB_PDF_AUTHOR.': '.utf8_decode($pdf_data['author']);
$pdf_data['date'] = NEWBB_PDF_DATE. ': '.formatTimestamp($pdf_data['date'],$pdf_config['dateformat']);
$pdf_data['url'] = NEWBB_PDF_URL. ': '.utf8_decode($pdf_data['url']);
...
$pdf->WriteHTML($pdf_config['scale'],$pdf_config['slogan']);
...
$pdf->WriteHTML($pdf_config['scale'],$pdf_data['title']);
 
if (!empty($pdf_data['subtitle'])){
        $pdf->WriteHTML($pdf_config['scale'],$puff);
        $pdf->SetFont($pdf_config['font']['subtitle']['family'],$pdf_config['font']['subtitle']['style'],$pdf_config['font']['subtitle']['size']);
        $pdf->WriteHTML($pdf_config['scale'],$pdf_data['subtitle']);
}
if (!empty($pdf_data["subsubtitle"])) {
        ...
        $pdf->WriteHTML($pdf_config['scale'],$pdf_data["subsubtitle"]);
}
 
$pdf->WriteHTML($pdf_config['scale'],$puff);
$pdf->SetFont($pdf_config['font']['author']['family'],$pdf_config['font']['author']['style'],$pdf_config['font']['author']['size']);
$pdf->WriteHTML($pdf_config['scale'],$pdf_data['author']);
$pdf->WriteHTML($pdf_config['scale'],$puff);
$pdf->WriteHTML($pdf_config['scale'],$pdf_data['date']);
$pdf->WriteHTML($pdf_config['scale'],$puff);
$pdf->WriteHTML($pdf_config['scale'],$pdf_data['url']);
$pdf->WriteHTML($pdf_config['scale'],$puff);
 
$pdf->SetTextColor(0,0,0);
$pdf->WriteHTML($pdf_config['scale'],$puffer);
 
$pdf->SetFont($pdf_config['font']['content']['family'],$pdf_config['font']['content']['style'],$pdf_config['font']['content']['size']);
$pdf->WriteHTML($pdf_config['scale'],$pdf_data['content']);
