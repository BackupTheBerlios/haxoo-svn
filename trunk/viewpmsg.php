 ...
 * @package         core
 * @since           2.0.0
 * @version         $Id: viewpmsg.php 5335 2010-09-18 23:56:24Z kris_fr $
 */
...
if (!is_object($xoopsUser)) {
    ...
    for($i = 0; $i < $total_messages; $i++) {
        ...
        if ($pm_arr[$i]->getVar('read_msg') == 1) {
            ...
            echo "<td class='aligntop width5 txtcenter'><img src='images/read.gif' alt='" . _PM_NOTREAD . "' /></td>\n";
        }
        echo "<td class='aligntop width5 txtcenter'><img src='images/subject/" . $pm_arr[$i]->getVar("msg_image", "E") . "' alt='' /></td>\n";
    ...
    if ($display == 1) {
        echo "<tr class='foot txtleft'><td colspan='6' align='left'><input type='button' class='formButton' onclick='javascript:openWithSelfMain(\"" . XOOPS_URL . "/pmlite.php?send=1\",\"pmlite\",450,380);' value='" . _PM_SEND . "' />&nbsp;<input type='submit' class='formButton' name='delete_messages' value='" . _PM_DELETE . "' />" . $GLOBALS['xoopsSecurity']->getTokenHTML() . "</td></tr></table></form>";
    } else {
        echo "<tr class='bg2 txtleft'><td class='txtleft' colspan='6'><input type='button' class='formButton' onclick='javascript:openWithSelfMain(\"" . XOOPS_URL . "/pmlite.php?send=1\",\"pmlite\",450,380);' value='" . _PM_SEND . "' /></td></tr></table></form>";
    }
